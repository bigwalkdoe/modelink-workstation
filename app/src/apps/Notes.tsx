...
const Notes: React.FC = () => {
  ...
  // Rich text formatting via modern Selection/Range API
  const formatInline = (tagName: string) => {
  ...
  const formatList = (tagName: 'ul' | 'ol') => {
    const sel = window.getSelection();
    ...
    const listParent = el instanceof HTMLLIElement ? el.closest('ul, ol') : null;
    if (listParent) {
      const parent = listParent.parentNode;
      if (!parent) return;
      const items = Array.from(listParent.children);
      const list = document.createElement(tagName);
      items.forEach((li) => {
        const newLi = document.createElement('li');
        newLi.innerHTML = (li as HTMLElement).innerHTML;
        list.appendChild(newLi);
      });
      parent.replaceChild(list, listParent);
    } else {
      const p = el instanceof HTMLParagraphElement ? el : el.closest('p');
      if (p) {
        const list = document.createElement(tagName);
        ...
        {activeNote ? (
          <>
            ...
            <div className="flex items-center gap-1 px-2 py-1.5 border-b shrink-0" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-titlebar)' }}>
              <button onClick={() => formatInline('b')} className="p-1.5 rounded hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]" title="Bold">
                <Bold size={13} />
              </button>
              <button onClick={() => formatInline('i')} className="p-1.5 rounded hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]" title="Italic">
                <Italic size={13} />
              </button>
              <button onClick={() => formatInline('u')} className="p-1.5 rounded hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]" title="Underline">
                <Underline size={13} />
              </button>
              <div className="w-px h-4 mx-1" style={{ background: 'var(--border-subtle)' }} />
              <button onClick={() => formatList('ul')} className="p-1.5 rounded hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]" title="Bullet List">
                <List size={13} />
              </button>
              <button onClick={() => formatList('ol')} className="p-1.5 rounded hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]" title="Numbered List">
                <ListOrdered size={13} />
              </button>
